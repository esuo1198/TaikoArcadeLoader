...
void InitializeLogger(LogLevel level, bool logToFile) {
    ...
    if(logToFile){
        ...
        if (!loggerInstance->logFile) LogMessage(__FILE__, __LINE__, "Failed to open log.txt for writing.", LOG_LEVEL_WARN);
    }
}
...
void LogMessage(const char* codeFile, int codeLine, const char* message, LogLevel messageLevel) {
    ...
    char logMessage[512];
    snprintf(logMessage, sizeof(logMessage), "%s(%d): %s", codeFile, codeLine, message);

    // Prepare the full log line (timestamp + type + message)
    char fullLogLine[600];
    snprintf(fullLogLine, sizeof(fullLogLine), "[%s.%03d] %s%s", timeStr, milliseconds, logType, logMessage);
...
LogLevel GetLogLevel(const std::string& logLevelStr) {
    ...
    } else if (logLevelStr == "HOOKS") {
        return LOG_LEVEL_HOOKS;
    }
